...
/**
 * `html-format`: used for `SandpackWithHTMLOutput` to format the HTML.
 */

/**
 * Shared package.json mounted at /projects/ so that `npm install`
 * only runs once during boot. Per-project subdirectories resolve
 * these packages via Node's standard upward module resolution.
 */
export const rootPackageJson = JSON.stringify(
  {
    name: "vite-react",
    private: true,
    type: "module",
...
export const rootPackageJsonLock = JSON.stringify({
  name: "vite-react",
  lockfileVersion: 3,
  requires: true,
  packages: {
...
/**
 * Per-project template files written into /projects/{id}/.
 * Does NOT include package.json — dependencies come from the
 * shared root installed during boot.
 */
export const viteReactTemplate: SandpackFiles = {
  "/vite.config.js": {
